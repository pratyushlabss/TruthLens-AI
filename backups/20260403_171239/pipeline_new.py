...
class ProductionRAGPipeline:
    ...
    def _compute_confidence_agreement_based(self, evidence: List[Dict[str, Any]], reasoning_confidence: float) -> float:
        """
        AGREEMENT-BASED CONFIDENCE: Use agreement ratio instead of raw similarity.
        
        Formula:
        - agreement_ratio = max(supports, contradicts) / (supports + contradicts + 1e-5)
        ...
        # Agreement ratio (higher when there's consensus)
        if supports + contradicts == 0:
            agreement_ratio = 0.5  # All neutral
        else:
            agreement_ratio = max(supports, contradicts) / (supports + contradicts + 1e-5)
        
        # Consistency (penalizes conflicting evidence)
        consistency = 1.0 - (abs(supports - contradicts) / max(1, total))
       
        # Combine signals
        computed_conf = (0.60 * agreement_ratio) + (0.40 * consistency)
        
        # Blend with LLM reasoning confidence
        final_confidence = (0.85 * computed_conf) + (0.15 * reasoning_confidence)
        
        # Clamp to valid range [0.05, 0.99]
        final_confidence = float(np.clip(final_confidence, 0.05, 0.99))
        
        logger.debug(f"[Confidence] Supports: {supports}, Contradicts: {contradicts}, Agreement: {agreement_ratio:.2f}, Final: {final_confidence:.2f}")
